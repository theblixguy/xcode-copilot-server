...
export async function patchSettings(proxy: ProviderMode | null, port: number, logger: Logger): Promise<void> {
...
export async function restoreSettings(proxy: ProviderMode | null, logger: Logger): Promise<void> {
  for (const patcher of resolvePatchers(proxy)) {
    try {
